    ...
    if (toggleBtn && formContainer) {
        toggleBtn.addEventListener('click', function() {
            if (formContainer.classList.contains('hidden')) {
                ...
                toggleBtn.innerHTML = '<i class="fas fa-times group-hover:scale-125 transition-transform duration-300"></i><span>إغلاق</span>';
            } else {
                formContainer.classList.add('hidden');
                toggleBtn.innerHTML = '<i class="fas fa-pen-alt group-hover:scale-125 transition-transform duration-300"></i><span>أضف تقييمك</span>';
            }
        });
    }
...
loadStoredTestimonials();
    ...
    function updateStoreStatus() {
        ...
        if (statusElement) {
            statusElement.innerText = isOpen ? "مفتوح الآن" : "مغلق حالياً";
